"""
Laura -- Canal Webhook (WhatsApp, GHL)
...
from laura_core import crear_agente
...
logger = logging.getLogger("laura_webhook")

PORT = int(os.getenv("WEBHOOK_PORT", "8086"))
GHL_API_KEY = os.getenv("GHL_API_KEY", "")
GHL_LOCATION_ID = os.getenv("GHL_LOCATION_ID", "")
GHL_BASE_URL = "https://services.leadconnectorhq.com"

# Tags que indican que el contacto ya fue procesado o cerrado
TAGS_PROCESADO = {"equity_con_lic", "equity_sin_lic", "lic_214", "contratado"}

MENSAJE_YA_REGISTRADO = (
    "Hola, ya tenemos tu información registrada en el Programa EQUITY. "
    "Un miembro de nuestro equipo estará en contacto contigo pronto. "
    "Si tienes alguna duda urgente, escríbenos directamente."
)
...
def _buscar_contacto_por_telefono(telefono: str) -> dict:
    """Busca contacto en GHL por teléfono."""
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if contactos:
                ...
                logger.info(f"[GHL] No se encontró contacto para {telefono_query}")
        else:
            logger.warning(f"[GHL] Error buscando contacto {telefono_query}: {r.status_code}")
    except Exception as e:
        logger.warning(f"[GHL] Excepción buscando contacto: {e}")
...
def _obtener_historial_ghl(contacto_id: str) -> str:
    """Obtiene los últimos 10 mensajes del contacto en GHL."""
    if not GHL_API_KEY or not contacto_id:
        return ""
                    ...
                    if r3.status_code == 200:
                        ...
                        if mensajes:
                            ...
                            for m in reversed(mensajes):
                                direccion = "Candidato" if m.get("direction") == "inbound" else "Laura"
...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def _handle_message(self):
        try:
            ...
            # Candidato ya procesado completamente → respuesta de asistencia
            if tags & TAGS_PROCESADO and "contratado" in tags:
                logger.info(f"[{session_id}] Candidato ya contratado → redirigiendo")
                response_data = {
                    "session_id": session_id,
                    "respuesta": MENSAJE_YA_REGISTRADO,
                    "duracion_segundos": 0,
                    "metadata": {
                        "agent": AGENT_NAME,
                        "canal": canal,
            ...
            # Inyectar historial GHL como contexto si existe
            if contacto_id:
                historial = _obtener_historial_ghl(contacto_id)
                if historial:
                    texto = (
                        f"[Contexto de conversación previa con este candidato:\n{historial}\n"
                        f"--- Mensaje actual ---]\n{texto}"
                    )
            ...
            logger.info(f"[{session_id}] Laura respondió en {duracion}s ({len(respuesta)} chars)")
